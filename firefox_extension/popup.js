...
/** Fetch with a manual timeout (AbortSignal.timeout not available in older Firefox). */
...
async function checkServer() {
  try {
    const r = await fetchWithTimeout(`${SERVER}/health`, {}, 2000);
    if (r.ok) {
      serverOk = true;
...
// Firefox MV2 uses browser.tabs.executeScript (not browser.scripting)
...
async function scrapeTab() {
  ...
  // Firefox MV2: executeScript resolves to array of raw return values
